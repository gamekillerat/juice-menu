...
namespace Big::UserInterface
{
	...
	{
		ResetInput();
		m_OpenKeyPressed = IsKeyPressed(VK_F7);
	...
	void YTDLoader(const char* ytd, const char* texture) {
		const char* file = ytd;
		std::ostringstream m_FilePath; m_FilePath << "C:\\Juice\\Theme\\Ui\\" << file;

		int textureID;
		if (FileExists(m_FilePath.str().c_str()))
			g_GameFunctions->pFileRegister(&textureID, m_FilePath.str().c_str(), true, file, false);
	...
	void UIManager::DrawHeader()
	{
		...
		case HeaderType::Gradient:
			...
			break;
		case HeaderType::YTD:
			YTDLoader("Textures.ytd", "Textures");
			DrawSprite(
				"Textures",
				"header",
				m_PosX,
	...
	{
		YTDLoader("Textures.ytd", "Textures");
		//DrawSprite("Texture", "", yourfloat1, yourfloat2, width, height, { 255, 255, 255, 255 }, 0); i restart ok?
		DrawRect( m_PosX, m_DrawBaseY + (m_OptionHeight / 2.f), m_Width, m_OptionHeight, m_OptionUnselectedBackgroundColor);
		DrawSprite("Textures", "scroller", m_PosX, m_DrawBaseY + (m_OptionHeight / 2.f), m_Width, m_OptionHeight, selected ? m_OptionSelectedBackgroundColor : m_OptionUnselectedBackgroundColor, 0);
		DrawLeftText(
			opt->GetLeftText(),
